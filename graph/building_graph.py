...
def build_graph(heterograph_data, for_summary=False):
    ...
    graph['token', 'similar_token', 'token'].edge_index = torch.tensor(
        [token_token_similarity_l, token_token_similarity_r], dtype=torch.long)
    graph['token', 'similar_token', 'token'].edge_attr = torch.reshape(torch.tensor(token_token_similarity_w), (-1, 1))

    graph['token', 'followed_by', 'token'].edge_index = torch.tensor([token_token_follow_l, token_token_follow_r], dtype=torch.long)
    ...
    graph['sent', 'similar_sent_rouge', 'sent'].edge_index = torch.tensor(
        [sent_sent_rouge_l, sent_sent_rouge_r], dtype=torch.long)
    ...
    graph['sent', 'similar_sent_sbert', 'sent'].edge_index = torch.tensor(
        [sent_sent_similarity_l, sent_sent_similarity_r], dtype=torch.long)
    graph['sent', 'similar_sent_sbert', 'sent'].edge_attr = torch.reshape(torch.tensor(sent_sent_similarity_w), (-1, 1))

    graph['sent', 'sent_contain_token', 'token'].edge_index = torch.tensor([sent_token_contain_l, sent_token_contain_r], dtype=torch.long)
    graph['sent', 'sent_contain_token', 'token'].edge_attr = torch.reshape(torch.tensor(sent_token_contain_w), (-1, 1))

    if not for_summary:
        graph['doc', 'doc_contain_sent', 'sent'].edge_index = torch.tensor([doc_sent_contain_l, doc_sent_contain_r], dtype=torch.long)
        graph['doc', 'doc_contain_sent', 'sent'].edge_attr = torch.reshape(torch.tensor(doc_sent_contain_w), (-1, 1))

        graph['doc', 'similar_doc_rouge', 'doc'].edge_index = torch.tensor([doc_doc_similarity_l, doc_doc_similarity_r], dtype=torch.long)
