...
class LEDModel(LEDPreTrainedModel):
    ...
    def encode_and_graph(
            self,
            encoder_outputs: LEDEncoderBaseModelOutput,
            attention_mask: torch.Tensor,
            heterograph: list,
            words_positions_source: list,
            sents_positions_source: list,
            docs_positions_source: Optional[list],
    ) -> Tuple[
        ...
        batch_size, seq_len, hidden_size = encoder_outputs.last_hidden_state.size()
        ...
        do_sag = docs_positions_source is not None
        ...
        for index, last_hidden_state_sample in enumerate(encoder_outputs.last_hidden_state):
            ...
            hg = heterograph[index % len(heterograph)].to(device)
            wp = words_positions_source[index % len(words_positions_source)].to(device)
            sp = sents_positions_source[index % len(sents_positions_source)].to(device)

            
            hg["token"].x = torch.index_select(last_hidden_state_sample, dim=0, index=wp)
            hg["sent"].x = torch.index_select(last_hidden_state_sample, dim=0, index=sp)

            if do_sag:
                dp = docs_positions_source[index % len(docs_positions_source)].to(device)
                hg["doc"].x = torch.index_select(last_hidden_state_sample, dim=0, index=dp)

            
            hgat = to_hetero(self.gat, hg.metadata(), aggr="sum").to(device)
            hg_undir = T.ToUndirected()(hg).to(device)
            hgat_output = hgat(hg_undir.x_dict, hg_undir.edge_index_dict)
            
            if do_sag:
               
                hgat_avg = (
                                   hgat_output["token"].mean(dim=0) +
                                   hgat_output["sent"].mean(dim=0) +
                                   hgat_output["doc"].mean(dim=0)
                           ) / 3.0  
            else:
            ...
            hgat_average_embeddings.append(hgat_avg)

            
            if do_sag:
                
                hg["sent"].x = hgat_output["sent"]

                
                sag_out = self.sag_pooling(
                    hg["sent"].x,
                    hg["sent", "similar_sent_sbert", "sent"].edge_index
                )
                embeddings_sents_compressed = sag_out[0]  
                nodes_sents_compressed = sag_out[-2]  

                
                if nodes_sents_compressed.numel() == 0:
                    
                    sag_avg = torch.zeros(hidden_size, device=device)
                    positions_sents_compressed = torch.tensor([], dtype=torch.long, device=device)
                    positions_tokens_compressed = torch.tensor([], dtype=torch.long, device=device)
                    positions_docs_compressed = torch.tensor([], dtype=torch.long, device=device)
                else:
                    
                    positions_sents_compressed = torch.index_select(sp, 0, nodes_sents_compressed)

                    
                    token_nodes_compressed = []
                    sent_token_ei = hg["sent", "sent_contain_token", "token"].edge_index
                    for l, r in zip(sent_token_ei[0], sent_token_ei[1]):
                        if int(l) in nodes_sents_compressed.tolist():
                            token_nodes_compressed.append(int(r))
                    if len(token_nodes_compressed) > 0:
                        token_nodes_compressed = torch.tensor(token_nodes_compressed, device=device)
                        positions_tokens_compressed = torch.index_select(wp, 0, token_nodes_compressed)
                        embeddings_tokens_compressed = torch.index_select(
                            last_hidden_state_sample, 0, positions_tokens_compressed
                        )  
                    else:
                        positions_tokens_compressed = torch.tensor([], dtype=torch.long, device=device)
                        embeddings_tokens_compressed = torch.zeros((1, hidden_size), device=device)  

                   
                    doc_sent_ei = hg["doc", "doc_contain_sent", "sent"].edge_index
                    nodes_docs_compressed = []
                    for l, r in zip(doc_sent_ei[0], doc_sent_ei[1]):
                        if int(r) in nodes_sents_compressed.tolist():
                            nodes_docs_compressed.append(int(l))
                    if len(nodes_docs_compressed) > 0:
                        nodes_docs_compressed = torch.tensor(nodes_docs_compressed, device=device)
                        positions_docs_compressed = torch.index_select(dp, 0, nodes_docs_compressed)
                        embeddings_docs_compressed = torch.index_select(
                            last_hidden_state_sample, 0, positions_docs_compressed
                        )  
                    else:
                        positions_docs_compressed = torch.tensor([], dtype=torch.long, device=device)
                        embeddings_docs_compressed = torch.zeros((1, hidden_size), device=device)

                    
                    sag_avg = (
                                      embeddings_sents_compressed.mean(dim=0) +
                                      embeddings_tokens_compressed.mean(dim=0) +
                                      embeddings_docs_compressed.mean(dim=0)
                              ) / 3.0  
                ...
                attn_mask_sample = attention_mask[index % attention_mask.size(0)]
                attn_mask_new = torch.zeros_like(attn_mask_sample, device=device)
                attn_mask_new[0] = 1
                attn_mask_new[-1] = 1
                for pos in positions_tokens_compressed.tolist():
                    attn_mask_new[pos] = 1
                for pos in positions_sents_compressed.tolist():
                    attn_mask_new[pos] = 1
                for pos in positions_docs_compressed.tolist():
                    attn_mask_new[pos] = 1

                sagpooling_average_embeddings.append(sag_avg)

            else:
                
                attn_mask_new = attention_mask[index % attention_mask.size(0)].clone().to(device)
                

            new_attention_masks.append(attn_mask_new)
            graph_hidden_states.append(last_hidden_state_sample)

        
        graph_hidden_states = torch.stack(graph_hidden_states, dim=0)  
        new_attention_mask = torch.stack(new_attention_masks, dim=0)  
        hgat_average_embeddings = torch.stack(hgat_average_embeddings, dim=0)  

        if do_sag:
           
            sagpooling_average_embeddings = torch.stack(sagpooling_average_embeddings,
                                                        dim=0)  
        else:
            sagpooling_average_embeddings = None

        
        encoder_outputs_graph = LEDEncoderBaseModelOutput(
            last_hidden_state=graph_hidden_states,
            hidden_states=None,
            attentions=None,
    ...
    def forward(
        ...
        if inputs_embeds is None and hasattr(self, '_last_inputs_embeds'):
            inputs_embeds = self._last_inputs_embeds
            
        self._last_inputs_embeds = inputs_embeds

        
        a, b, _ = inputs_embeds.size()
        ...
        for i in range(1):
            ...
            img_feat_tensor = torch.tensor(img_feat,
                                       dtype=inputs_embeds.dtype,
                                       device=inputs_embeds.device)  

            
            img_feat_tensor= img_feat_tensor.view(1, 1, -1)

            
            img_feat_tensor =  img_feat_tensor.expand(a, b,  img_feat_tensor.size(-1))
            img_cat_tensor=torch.cat((inputs_embeds, img_feat_tensor), dim=-1)
            image_out=self.linear1(img_cat_tensor)
            inputs_embeds=image_out

        for i in range(1):
            ad_feat = audio_feat[i]
            if len(ad_feat) == 0:
               
                continue

            
            audio_feat_tensor = torch.tensor(ad_feat,
                                       dtype=inputs_embeds.dtype,
                                       device=inputs_embeds.device)  

            
            audio_feat_tensor = audio_feat_tensor.view(1, 1, -1)

            audio_feat_tensor = audio_feat_tensor.expand(a, b, audio_feat_tensor.size(-1))
            audio_cat_tensor = torch.cat((image_out, audio_feat_tensor), dim=-1)
            audio_out=self.linear2( audio_cat_tensor)
            inputs_embeds=audio_out

        if encoder_outputs is None:
            encoder_outputs_plain = self.encoder(
                input_ids=input_ids,
                attention_mask=attention_mask,
            ...
            encoder_outputs_graph, new_attention_mask, hgat_avg, sagpooling_avg = self.encode_and_graph(
                encoder_outputs=encoder_outputs_plain,
                attention_mask=attention_mask,
            ...
            encoder_outputs = encoder_outputs_graph

            
        elif return_dict and not isinstance(encoder_outputs, LEDEncoderBaseModelOutput):
            encoder_outputs_plain = LEDEncoderBaseModelOutput(
            ...
            hgat_avg = None
            sagpooling_avg = None
            encoder_outputs_graph = encoder_outputs
        ...
        return LEDSeq2SeqModelOutput(
            ...
            encoder_last_hidden_state=encoder_outputs_graph.last_hidden_state,
            encoder_hidden_states=encoder_outputs_graph.hidden_states,
            encoder_attentions=encoder_outputs_graph.attentions,
            encoder_global_attentions=encoder_outputs_graph.global_attentions,
            mgat_outputs=hgat_avg,
            sagpooling_outputs=sagpooling_avg,
        )
...
class LEDForConditionalGeneration(LEDPreTrainedModel):
    ...
    def prepare_inputs_for_generation(
        ...
        cross_attn_head_mask: Optional[torch.Tensor] = None,
        use_cache: Optional[bool] = None,
        encoder_outputs: Optional[LEDEncoderBaseModelOutput] = None,
        heterograph: Optional[List[HeteroData]] = None,
        words_positions_source: Optional[List[torch.LongTensor]] = None,
        sents_positions_source: Optional[List[torch.LongTensor]] = None,
        docs_positions_source: Optional[List[torch.LongTensor]] = None,
        ...
        **kwargs,
        ...
        if encoder_outputs is None:
            with torch.no_grad():
                ...
                encoder_outputs_plain = self.led.encoder(
                ...
                encoder_outputs_graphed, new_attn_mask = self.led.encode_and_graph(
                    encoder_outputs=encoder_outputs_plain,
                    attention_mask=attention_mask,
                    heterograph=heterograph,
                    words_positions_source=words_positions_source,
            ...
            encoder_outputs = encoder_outputs_graphed
    ...
    def _prepare_encoder_decoder_kwargs_for_generation(
        ...
        hg = model_kwargs.pop("heterograph", None)
        wps = model_kwargs.pop("words_positions_source", None)
        sps = model_kwargs.pop("sents_positions_source", None)
        dps = model_kwargs.pop("docs_positions_source", None)

        art      = model_kwargs.pop("article_name", None)
        vfile    = model_kwargs.pop("video_file", None)
        bnds     = model_kwargs.pop("bounds", None)
        vdur     = model_kwargs.pop("video_duration", None)
        img_feat = model_kwargs.pop("image_feat", None)
        aud_feat = model_kwargs.pop("audio_feat", None)
        plc      = model_kwargs.pop("place", None)

        
        encoder_kwargs = super()._prepare_encoder_decoder_kwargs_for_generation(
            input_ids, model_kwargs, encoder_outputs
        )

        if hg is not None:
            encoder_kwargs["heterograph"] = hg
        if wps is not None:
            encoder_kwargs["words_positions_source"] = wps
        if sps is not None:
            encoder_kwargs["sents_positions_source"] = sps
        if dps is not None:
            encoder_kwargs["docs_positions_source"] = dps
        if art is not None:
            encoder_kwargs["article_name"] = art
        if vfile is not None:
            encoder_kwargs["video_file"] = vfile
        if bnds is not None:
            encoder_kwargs["bounds"] = bnds
        if vdur is not None:
            encoder_kwargs["video_duration"] = vdur
        if img_feat is not None:
            encoder_kwargs["image_feat"] = img_feat
        if aud_feat is not None:
            encoder_kwargs["audio_feat"] = aud_feat
        if plc is not None:
            encoder_kwargs["place"] = plc

        
        if hg is not None:
            model_kwargs["heterograph"] = hg
        if wps is not None:
            model_kwargs["words_positions_source"] = wps
        if sps is not None:
            model_kwargs["sents_positions_source"] = sps
        if dps is not None:
            model_kwargs["docs_positions_source"] = dps
        if art is not None:
            model_kwargs["article_name"] = art
        if vfile is not None:
            model_kwargs["video_file"] = vfile
        if bnds is not None:
            model_kwargs["bounds"] = bnds
        if vdur is not None:
            model_kwargs["video_duration"] = vdur
        if img_feat is not None:
            model_kwargs["image_feat"] = img_feat
        if aud_feat is not None:
            model_kwargs["audio_feat"] = aud_feat
        if plc is not None:
            model_kwargs["place"] = plc
